...
struct Node {
    ...
    Node(int val = -1) : data(val), next(nullptr), prev(nullptr) {}
};
...
public:
    ...
    Node* search(int key) {
        nil->data = key;  // Store the key in the sentinel
        Node* x = nil->next;  // Start at the head of the list

        while (x->data != key) {
            x = x->next;
        }

        if (x == nil)  // Found key in the sentinel
            return nullptr;  // Key was not really in the list
        else
            return x;  // Found key in the list
    }
