...
// Comparator function untuk qsort (mengurutkan interval berdasarkan nilai awal)
...
int** merge(int** intervals, int intervalsSize, int* intervalsColSize, 
    ...
    // Urutkan interval berdasarkan nilai awal menggunakan qsort
    qsort(intervals, intervalsSize, sizeof(int*), cmpIntervals);
    
    // Alokasikan array output dengan kapasitas maksimal sama dengan intervalsSize
    int** merged = (int**)malloc(intervalsSize * sizeof(int*));
    int mergedCount = 0;
    
    // Salin interval pertama ke dalam merged sebagai interval awal
    ...
    for (int i = 1; i < intervalsSize; i++) {
        ...
        if (intervals[i][0] <= merged[mergedCount - 1][1]) {
            // Update nilai akhir menjadi nilai maksimum antara interval yang sudah ada dan interval baru
            if (intervals[i][1] > merged[mergedCount - 1][1])
                merged[mergedCount - 1][1] = intervals[i][1];
        } else {
            // Jika tidak tumpang tindih, alokasikan interval baru pada merged
            merged[mergedCount] = (int*)malloc(2 * sizeof(int));
    ...
    *returnSize = mergedCount;
    // Alokasikan returnColumnSizes dengan tepat: tiap interval berukuran 2
